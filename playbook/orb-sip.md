...
- **ingresso:** stop buy 1 tick sopra il massimo del range (OR5 o OR15) se la candela è verde. Lato ribassista: **niente short** con questo conto, si usa una put o (su indici e settori) l'ETF inverso, come nella variante `orb-inverso`.
- **stop:** 10-25% dell'ATR a 14 giorni dall'ingresso, oppure l'estremo opposto del range se più vicino.
- **uscita:** default `fixed`, target 2R con bracket a gambe GTC. Alternative ammesse: `breakeven` a +1R, oppure `trailing` con trailing stop di Alpaca (senza target). Comunque flat entro le 15:50 ET.
- **dimensione:** secondo lo stato del setup e le fasce di `rischio/sizing.md`.
- **alternative:** titolo troppo costoso → call o debit spread con la stessa tesi (`opt-catalyst`); ingresso troppo lontano dallo stop → varianti `orb-retest` o `orb15-conferma` in `orb-varianti.md`.
